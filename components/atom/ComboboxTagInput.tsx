...
const ComboboxTagInput = <X,>({
  ...
  return (
    <Field id={name}>
      <Combobox name={name} value={selectedItems} onChange={selectOption} multiple>
          <div className={InputWrapperVariants({ variant })}>
              <Combobox.Input
                  ...
                  value={query}
              ></Combobox.Input>
              <Label className={"order-1 text-text text-primary-600 peer-focus:text-primary-950 data-[open]:text-primary-950"}>{label}</Label>
              <Combobox.Options
                  anchor="bottom"
                  className={cn(
              ...
              >
                  {dropdownItems.map((val, idx) => (
                    ...
                    <span
                        className={cn(
                            "text-primary-950 text-text"
                        )}
                    >
                      {autocompleteFunctions.renderFunction(val)}
                    </span>
                        <CheckIcon className={'invisible group-data-[selected]:visible'} size="default" aria-hidden="true"/>
                      </Combobox.Option>
                  ))}
              </Combobox.Options>
              {selectedItems.length == 0 ? (
                  <Description className='order-3 mt-0.5 font-body text-primary-600 text-subtext peer-focus:text-primary-950'>{subtext}</Description>) :
