...
const display_data: {
  modal_text: localized_string;
  subtext: localized_string;
...
const MathpixModal: FC<MathpixModalProps> = ({
  locale = "en",
  callback,
  buttonVariants: { variant, size } = { theme: "dark", variant: "tertiary" },
}) => {
  ...
  return (
    <>
      <Button variant={variant} size={size} type={"button"} onClick={openModal}>
        Create from picture
        <AICreateIcon aria-hidden="true" size="default" />
      </Button>

      <Modal
        title={display_data.modal_text[locale]!}
        isOpen={isOpen}
        close={closeModal}
      >
        <FileInput
          ...
          className="mb-1"
          fileIcons={["jpg", "png"]}
          subtext={display_data.subtext[locale]!}
        />
        <Button onClick={handleUpload} className="w-full">
          {display_data.button_Text[locale]!}
        </Button>
      </Modal>
    </>
  );
