...
export const infoToast = (message: string) => {
    sonnerToast.custom( (t) =>
        <div className='flex flex-row bg-white p-2 gap-2 items-center rounded-md border border-primary-950'>
            ...
            <span className='text-primary-950 text-text flex-grow whitespace-nowrap overflow-hidden'>{message}</span>
            <div className='text-primary-950' onClick={() => toast.dismiss(t)}><XIcon size='default' /></div>
        </div>
        );
};
...
// Promise Toast
export const promiseToast = (promise: Promise<any>, messages: { pending: string, success: string, error: string }) => {
    ...
    promise.then(() => {
        sonnerToast.custom( (t) =>
            <div className='flex flex-row bg-white p-2 gap-2 items-center rounded-md border border-success'>
                <SuccessIcon size='default' className='text-success' />
                <span className='text-primary-950 text-text flex-grow whitespace-nowrap overflow-hidden'>{messages.success}</span>
                <div className='text-primary-950' onClick={() => toast.dismiss(t)}><XIcon size='default' /></div>
            </div> ,{id: toastId}
        );
    }).catch(() => {
        sonnerToast.custom( (t) =>
            <div className='flex flex-row bg-white p-2 gap-2 items-center rounded-md border border-error'>
                <ErrorIcon size='default' className=' text-error' />
                <span className='text-primary-950 text-text flex-grow '>{messages.error}</span>
                <div className='text-primary-950' onClick={() => toast.dismiss(t)}><XIcon size='default' /></div>
            </div> ,{id: toastId}
        );
    });
